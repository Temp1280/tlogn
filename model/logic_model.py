...
class GNNModel(torch.nn.Module):
    def __init__(self, emb_dim, gcn_dim, num_rel, act=lambda x: x, max_nodes=50, reduce='sum'):
        ...
        self.W_h = nn.Linear(emb_dim*2, emb_dim)
        self.line_e_ts = nn.Linear(gcn_dim*2, emb_dim)
    ...
    def forward(self, query_time, query_emd, temp_neighbors_facts, ent_emds, rel_emds,
                hidden_node, hidden_rel, hidden_time, att_agg, time_encoder, line_time, gru1, gru2, batch_size, device, max_nodes):  # X*6 b,n,h,r,t,ts  N*3 X
...
class GLogicalLayer(torch.nn.Module):  # 规则解码相关
    ...
    def forward(self, ent_emds, rel_emds, query_head, query_rel, query_time, dataset, time_encoder, device, args):
        ...
        query_time_emd = time_encoder(query_time)
        ...
        for i in range(self.n_layer):
            ...
            tail_nodes, hidden_node, hidden_rel, hidden_time, att_agg = self.gnn_layers[i](query_time, query_emd, temp_neighbors_facts, ent_emds, rel_emds, hidden_node, hidden_rel,
                                        hidden_time, att_agg, time_encoder, self.line_time, self.gru1, self.gru2, batch_size, device, max_nodes)  # N*D
